...
export const TOPOGRAPHIC_GRID_COLUMNS = TOPOGRAPHIC_STYLE.detail.columns;
export const TOPOGRAPHIC_GRID_ROWS = TOPOGRAPHIC_STYLE.detail.rows;
export const TOPOGRAPHIC_GRID_PADDING = Math.round(
  Math.max(TOPOGRAPHIC_GRID_COLUMNS, TOPOGRAPHIC_GRID_ROWS) *
    TOPOGRAPHIC_STYLE.canvas.fieldPaddingRatio,
);

const FIELD_COLUMNS = TOPOGRAPHIC_GRID_COLUMNS + TOPOGRAPHIC_GRID_PADDING * 2;
const FIELD_ROWS = TOPOGRAPHIC_GRID_ROWS + TOPOGRAPHIC_GRID_PADDING * 2;
...
export const TOPOGRAPHIC_MOTION = {
  scale: TOPOGRAPHIC_STYLE.motion.scale,
  durationRange: TOPOGRAPHIC_STYLE.motion.durationRange,
  delayPerLine: TOPOGRAPHIC_STYLE.motion.delayPerLine,
} as const;
...
const createFieldCoordinates = () => {
  ...
  for (let index = 0; index < FIELD_CELL_COUNT; index += 1) {
    xCoordinates[index] =
      ((index % FIELD_COLUMNS) - TOPOGRAPHIC_GRID_PADDING) /
      (TOPOGRAPHIC_GRID_COLUMNS - 1);
    yCoordinates[index] =
      (Math.floor(index / FIELD_COLUMNS) - TOPOGRAPHIC_GRID_PADDING) /
      (TOPOGRAPHIC_GRID_ROWS - 1);
  }
